...
- Docs de-named: the pre-existing 0.1.0 illustrative examples that used real client/teammate names (`DESIGN.md` §10 packet example, `README.md`, `core/intake-scan.md`, and the `scribe-share`/`scribe-inbox`/`scribe-list`/`scribe-import` command docs + `project-instructions.md`) were genericized to **roles** — not fictional people, not real names: e.g. `the operator` / `restaurant`, `the designer` / `nonprofit`, `the founder` / `field-ops`. Author byline (`Apollo Mondesir / Rocksteady Consulting`) intentionally retained. No code, structure, or behavior changed.
